...
NSG_NAME="sonarqube_nsg-${STAGE}"

echo "Updating Network security rule in ${NSG_NAME} to allow SSH Access for ${MY_IP}"
az network nsg rule update -g "sonarqube-app-${STAGE}" --nsg-name "${NSG_NAME}" -n "ssh" \
...
az network nsg rule list -g "sonarqube-app-${STAGE}" --nsg-name "${NSG_NAME}" --subscription "${TF_VAR_subscription_id}"
